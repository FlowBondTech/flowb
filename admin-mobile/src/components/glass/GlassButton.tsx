...
export function GlassButton({
  ...
  const inner = (
    <View
      style={[
        styles.row,
        { height: preset.height, paddingHorizontal: preset.paddingHorizontal },
      ]}>
      {loading ? (
        ...
        <>
          ...
          <Text
            style={[styles.label, { fontSize: preset.fontSize, color: textColor }]}
            numberOfLines={1}>
            {title}
          </Text>
        </>
  ...
  const renderBody = () => {
    switch (variant) {
      ...
      case 'secondary':
        return (
          <View
            style={[styles.base, glassStyle('subtle'), disabled && styles.disabled]}>
            {inner}
          </View>
        );
      case 'ghost':
        return (
          <View style={[styles.base, styles.ghost, disabled && styles.disabled]}>
            {inner}
          </View>
        );
    }
...
const styles = StyleSheet.create({
  ...
  label: { fontWeight: '600', letterSpacing: 0 },
